...
if [ -z  "`lsmod | grep bitmain_lcd`"  ]; then
	echo "No bitmain-lcd"
	insmod /lib/modules/3.8.13/kernel/drivers/bitmain/bitmain_lcd.ko SLEEP_TIME=3 testing_hash=1
else
	echo "Have bitmain-lcd"
	rmmod -f bitmain_lcd.ko
	sleep 1
	insmod /lib/modules/3.8.13/kernel/drivers/bitmain/bitmain_lcd.ko SLEEP_TIME=3 testing_hash=1
fi
...
if [ -z  "`lsmod | grep bitmain_spi`"  ]; then
	...
	insmod /lib/modules/3.8.13/kernel/drivers/bitmain/bitmain_spi.ko
else
 	echo "Have bitmain-asic"
	rmmod -f bitmain_spi.ko
	sleep 1
	insmod /lib/modules/3.8.13/kernel/drivers/bitmain/bitmain_spi.ko fpga_ret_prnt=0 rx_st_prnt=0
fi
#beep gpio0_20
beep=20
