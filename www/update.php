...
if (isset($_POST['zwillamod_version']) &&
    ($_POST['zwillamod_version'] != CURRENT_VERSION) &&
    ...
    $newVersion = strip_tags($_POST['zwillamod_version']);
    ...
    if ($settings['general']['updates']['enabled'] == 1) {
        ...
        $curl = curl_init();
        $fop = fopen($currentDir . $updateDir .'.zip', 'w');
        ...
        foreach ($files as $file) {
            if ($file->getFilename() === '.' || $file->getFilename() === '..' || strpos($file->getRealPath(), DIRECTORY_SEPARATOR . "/") !== false) {
                continue;
            }
        ...
        $failedFolders = array();
        foreach ($files as $file) {
            if ($file->getFilename() === '.' || $file->getFilename() === '..' || strpos($file->getRealPath(), DIRECTORY_SEPARATOR.'update') !== false || strpos($file->getRealPath(), DIRECTORY_SEPARATOR.'.update') !== false || strpos($file->getRealPath(), DIRECTORY_SEPARATOR . "/") !== false) {
                continue;
            }
            $realFilePath = $file->getRealPath();
            if ($file->isDir()){
              ...
              <div class="stat-pair">
                <div class="stat-label">
                  <form method="POST" target="cg_update">
                    ...
                    <p>The version of cryptoGlance Zwilla Mod that you're running <span class="orange">(<?php echo CURRENT_VERSION ?>)</span> can be updated to <span class="green"><?php echo $_COOKIE['zwillamod_version'] ?></span>.</p>
                    <p>It is advised that you backup your <span class="blue">\/</span> folder prior to running the update process.</p>
                    ...
                    <input type="hidden" name="zwillamod_version" value="<?php echo $_COOKIE['zwillamod_version'] ?>" />
