...
class Miners_Cgminer extends Miners_Abstract {
    ...
    public function summary() {
        ...
        return array(
            ...
            'ride on' => $_COOKIE['makerunningmode'],
            'accepted' => array(
            ...
            'best_share' => number_format($this->_summary['Best Share'], 0, ".", ","),
            // number_format($userData['bestshare'], 1, ".", ",");
            // 'best_share' => number_format($this->_summary['Best Share'], 1, ".", ","),
            ...
            foreach ($this->_eStats as $eStats) {
                ...
                for ($i=1; $eStats['miner_count'] >=$i; $i++) {
                    if (strpos($eStats['chain_acs'.$i], 'x') !== false) {
                      ...
                      if ($_COOKIE['makerunningmode'] = 0) {$zMode = "Z-Mod480"; $this->restart(); }
					  if ($_COOKIE['makerunningmode'] = 1) {$zMode = "Z-Mod492";}
				 	  if ($_COOKIE['makerunningmode'] = 2) {$zMode = "Old-Mode"; $this->restart(); }
