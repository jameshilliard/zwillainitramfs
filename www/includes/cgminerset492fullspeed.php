...
$cgApiDesc			=	"\"api-description\": \"cgminer zwilla mod Overkill\",\r\n";
...
$cgApiListen = "true"; // without that the frontend is without any function!
    ...
    $cgminerconf492o = ( $cgStartconf1 . $cgPoolone . $cgPoolWorker1 . $cgPoolPass13 .
    ...
    if ($settings['general']['activate']['minerFanAuto'] == "true") {
        $cgminerconf492o = $cgminerconf492o . $cgMinerFanAutoS7;
    }
    else {
        $cgminerconf492o = $cgminerconf492o . $cgMinerFanMaxS7;
    }


    $cgminerconf492o = $cgminerconf492o . $cgFallback492 . $cgEndConf;


    $cgminerexternalconf492 = "--bitmain-dev /dev/bitmain-asic --bitmain-checkn2diff --bitmain-hwerror --queue 8192 ";
    ...
    $file492conf = "/config/cgminer/start492.txt";
    if (file_exists($file492conf)) {
        $file492conf = "/config/cgminer/start492.txt";
        unlink($file492conf);

    }

    $filestartexternal492 = fopen("/config/cgminer/start492overkill.txt","w+");
    ...
    fwrite($file492conf, $cgminerconf492o);
    fclose($file492conf);
