...
check0p3=`cat /etc/mtab | grep "mmcblk0p4"`
#if [ $? -eq 0 ]; then
	echo $check0p4

	if [ ""x = "$check0p3"x ] ; then
		echo "mounting config"
		mount /dev/mmcblk0p4 /config
	fi
#fi
check0p4=`cat /etc/mtab | grep "mmcblk0p4"`
if [ ""x = "$check0p4"x ] ; then
    ...
    echo "mounting mmcblk0p2"
    mkdir /mnt/mmc1
    mount /dev/mmcblk0p2 /mnt/mmc1
    touch /mnt/mmc1/bingeladen.txt
fi
